...
namespace canard {
  ...
  namespace detail {
    ...
    struct asio_handler_hook_propagation_base
    {
        ...
        -> void*
      {
        ...
        return asio_handler_allocate(size, std::addressof(h->handler()));
      }

      friend void asio_handler_deallocate(
          void* const pointer, std::size_t const size, Derived* const h)
      {
        using boost::asio::asio_handler_deallocate;
        asio_handler_deallocate(pointer, size, std::addressof(h->handler()));
      }
        ...
        -> bool
      {
        ...
        return asio_handler_is_continuation(std::addressof(h->handler()));
      }
    };
    ...
    : detail::asio_handler_hook_propagation_base<Derived>
  {
    ...
    friend void asio_handler_invoke(Function&& function, Derived* const h)
    {
      ...
      asio_handler_invoke(
          std::forward<Function>(function), std::addressof(h->handler()));
    }
  };
