...
BOOST_AUTO_TEST_CASE(scalar_hton_test)
{
    ...
    BOOST_CHECK_EQUAL(detail::hton(integer), 0x3412);
}

BOOST_AUTO_TEST_CASE(ofp_header_hton_test)
{
    auto const header = detail::ofp_header{OFP_VERSION, OFPT_HELLO, 0x3456, 0x12345678};
    auto const norder_header = detail::hton(header);
...
BOOST_AUTO_TEST_CASE(scalar_ntoh_test)
{
    ...
    BOOST_CHECK_EQUAL(detail::ntoh(integer), 0x0807060504030201);
}

BOOST_AUTO_TEST_CASE(ofp_header_ntoh_test)
{
    auto const header = detail::ofp_header{OFP_VERSION, OFPT_HELLO, 0x3456, 0x12345678};
    auto const horder_header = detail::ntoh(header);
