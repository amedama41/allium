...
BOOST_AUTO_TEST_SUITE(instantiation_test)
    ...
    {
        auto const oxm = oxm_match::eth_type{0x0806};
        ...
        auto const sut_field = any_cast<oxm_match::eth_type>(&sut.oxm_match_field());
    ...
    {
        auto sut = actions::set_field{oxm_match::ipv6_src{address_v6{{{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0xd, 0x0e, 0x0f}}}}};
    ...
    {
        auto oxm = any_oxm_match_field{oxm_match::vlan_vid{0x0fff}};
        ...
        BOOST_CHECK_EQUAL(sut.length() % 8, 0);
        BOOST_CHECK_EQUAL(sut.oxm_match_field().oxm_type(), oxm_match::vlan_vid::oxm_type());
        auto const sut_field = any_cast<oxm_match::vlan_vid>(&sut.oxm_match_field());
        BOOST_REQUIRE(sut_field);
...
BOOST_AUTO_TEST_SUITE(assignment_test)
    ...
    {
        auto sut = actions::set_field{oxm_match::ipv4_src{address_v4{0x7f000001}}};
        auto copy = actions::set_field{oxm_match::ipv4_src{address_v4{0x7f000002}}};

        copy = sut;

        BOOST_CHECK_EQUAL(copy.type(), sut.type());
        BOOST_CHECK_EQUAL(copy.length(), sut.length());
        BOOST_CHECK_EQUAL(copy.oxm_match_field().oxm_type(), oxm_match::ipv4_src::oxm_type());
        auto const field = any_cast<oxm_match::ipv4_src>(&copy.oxm_match_field());
        BOOST_REQUIRE(field);
        BOOST_CHECK_EQUAL(field->oxm_value(), address_v4{0x7f000001});
    }
...
BOOST_AUTO_TEST_CASE(encode_decode_test)
{
    ...
    auto const sut = actions::set_field{oxm_match::ipv4_src{address_v4{0x7f000001}}};
    ...
    BOOST_CHECK_EQUAL(decoded_action.oxm_match_field().oxm_type(), oxm_match::ipv4_src::oxm_type());
    auto const decode_action_field = any_cast<oxm_match::ipv4_src>(&sut.oxm_match_field());
    BOOST_REQUIRE(decode_action_field);
    BOOST_CHECK_EQUAL(decode_action_field->oxm_value(), address_v4{0x7f000001});
}
