...
BOOST_AUTO_TEST_SUITE(vlan_pcp_test)
    ...
    BOOST_DATA_TEST_CASE(
            ...
            , value)
    {
        auto const sut = match::vlan_pcp::create(value);
    ...
    BOOST_DATA_TEST_CASE(
            ...
            , bdata::make(std::vector<std::uint8_t>{0x00, 0x07})
            ^ bdata::make(std::vector<std::uint8_t>{0x01, 0x07})
            , value, mask)
    {
        auto const sut = match::vlan_pcp::create(value, mask);
    ...
    BOOST_DATA_TEST_CASE(
            ...
            , value)
    {
        BOOST_CHECK_THROW(match::vlan_pcp::create(value), std::runtime_error);
    }

    BOOST_AUTO_TEST_CASE(has_mask_create_failure_test)
    {
        BOOST_CHECK_THROW(
                match::vlan_pcp::create(0x01, 0x00), std::runtime_error);
    }
